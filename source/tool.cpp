...
{
    return LXfTMOD_I0_INPUT | LXfTMOD_DRAW_3D | LXfTMOD_AUTORESET;
}
...
LxResult CUVTransform::HitPolygon(CLxUser_VectorStack& vec)
{
    ...
    {
        printf("[HIT] poly %p wPos %f %f %f\n", hit.pol, hit.wPos[0], hit.wPos[1], hit.wPos[2]);
	    CLxUser_Mesh mesh;
        CLxUser_Item item;
...
LxResult CUVTransform::tmod_Down(ILxUnknownID vts, ILxUnknownID adjust)
{
...
#endif
    printf("[DOWN] pos %f %f axis %f %f %f part (%x)\n", 
        m_mousePos[0], m_mousePos[1], m_axis[0], m_axis[1], m_axis[2], ipak->part);
    m_part = ipak->part;
    ...
    switch (m_part)
    {
        case HANDLE_TRANS_U:
            ...
            printf("TRANS_U %f %f %f\n", m_space.m_view_matrix[0][0], m_space.m_view_matrix[0][1], m_space.m_view_matrix[0][2]);
            if (view->type == LXi_VIEWTYPE_3D)
                LXx_VSET3(dir, m_space.m_view_matrix[0][0], m_space.m_view_matrix[0][1], m_space.m_view_matrix[0][2]);
            else
                LXx_VUNIT(dir, 0);
        ...
        case HANDLE_TRANS_V:
            ...
            printf("TRANS_V %f %f %f\n", m_space.m_view_matrix[1][0], m_space.m_view_matrix[1][1], m_space.m_view_matrix[1][2]);
            if (view->type == LXi_VIEWTYPE_3D)
                LXx_VSET3(dir, m_space.m_view_matrix[1][0], m_space.m_view_matrix[1][1], m_space.m_view_matrix[1][2]);
            else
                LXx_VUNIT(dir, 1);
            epkt.SetLinearConstraint(vts, m_mousePos, dir);
            break;
        ...
        case HANDLE_CENTER:
            ...
            if (view->type == LXi_VIEWTYPE_3D)
            {
                LXx_VSET3(dir, m_space.m_view_matrix[2][0], m_space.m_view_matrix[2][1], m_space.m_view_matrix[2][2]);
        ...
        case HANDLE_ROTATE:
            ...
            m_rotHandle.MouseDown(viewId, m_center0, m_axis, m_mousePos);
...
void CUVTransform::tmod_Move(ILxUnknownID vts, ILxUnknownID adjust)
{
    ...
    CLxVector pos1 = CLxVector(pos);
    //printf("[MOVE] pos1 %f %f %f pos0 %f %f %f\n", pos1[0], pos1[1], pos1[2], pos0[0], pos0[1], pos0[2]);
    if (view->type == LXi_VIEWTYPE_3D)
    {
        pos0 *= m_space.m_view_matrix_inv;
        pos1 *= m_space.m_view_matrix_inv;
    }
    double trans_u = pos1[0] - pos0[0];
    double trans_v = pos1[1] - pos0[1];
    printf("[MOVE] trans_u %f trans_v %f\n", trans_u, trans_v);
    switch (m_part)
    {
        case HANDLE_TRANS_U:
        ...
        case HANDLE_CENTER:
            ...
            break;
        case HANDLE_ROTATE:
            m_rotHandle.MouseMove(pos);
            m_rotHandle.GetAngles(2, m_xfrm, &m_sAngle, &m_eAngle);
            at.SetFlt(ATTRa_ANGLE, m_angle0 + m_rotHandle.m_angle);
        //printf("[MOVE] cx %d cy %d sAngle %f eAngle %f\n", spak->cx, spak->cy, m_sAngle*LXx_RAD2DEG, m_eAngle*LXx_RAD2DEG);
            break;
        default:
